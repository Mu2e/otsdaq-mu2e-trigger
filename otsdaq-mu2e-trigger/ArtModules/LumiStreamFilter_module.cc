  ...
  public:
	struct Config
	{
		fhicl::Atom<int> diagLevel{
		    fhicl::Name("diagLevel"), fhicl::Comment("diagnostic Level"), 0};
		fhicl::OptionalAtom<art::InputTag> caloTag{
		    fhicl::Name("caloTag"), fhicl::Comment("Calo intensity info Tag")};
		fhicl::OptionalAtom<art::InputTag> timeClusterTag{
		    fhicl::Name("timeClusterTag"),
		    fhicl::Comment("Time cluster intensity info Tag")};
		fhicl::OptionalAtom<art::InputTag> trackerTag{
		    fhicl::Name("trackerTag"), fhicl::Comment("Tracker intensity info Tag")};
		...
		fhicl::OptionalAtom<int>           eventFreq{
            ...
            fhicl::Comment("Frequency of event data product writing, defaulting to "
		                             "sub-runs if not set")};
		fhicl::Atom<bool> passFirst{
		    fhicl::Name("passFirst"),
		    fhicl::Comment(
		        "Pass first event per sub-run to ensure the output file is prepared"),
		    false};
		fhicl::Atom<int> duplications{fhicl::Name("duplications"),
		                              fhicl::Comment("Duplicate input data N times"),
		                              0};
	};
    ...
    , _headers(nullptr)
{
	...
	{
		_caloInfos =
		    std::unique_ptr<mu2e::IntensityInfosCalo>(new mu2e::IntensityInfosCalo);
		if(!_useSubruns)
	...
	if(_useTimeCluster)
	{
		_timeClusterInfos = std::unique_ptr<mu2e::IntensityInfosTimeCluster>(
		    new mu2e::IntensityInfosTimeCluster);
		if(!_useSubruns)
	...
	if(_useTracker)
	{
		_trackerInfos = std::unique_ptr<mu2e::IntensityInfosTrackerHits>(
		    new mu2e::IntensityInfosTrackerHits);
		if(!_useSubruns)
...
bool LumiStreamFilter::filter(art::Event& event)
{
	...
	if(_useCalo)
	{
		...
		if(!event.getByLabel(_caloTag, caloH) || !caloH.product())
		{
			...
			_caloInfos->push_back(
			    mu2e::
			        IntensityInfoCalo());  // add an empty one to maintain the list alignment
		}
		else
		{
	...
	if(_useTimeCluster)
	{
		...
		if(!event.getByLabel(_timeClusterTag, timeClusterH) || !timeClusterH.product())
		{
			...
			_timeClusterInfos->push_back(
			    mu2e::
			        IntensityInfoTimeCluster());  // add an empty one to maintain the list alignment
		}
		else
		{
			const auto timeClusterInfo = timeClusterH.product();
			_timeClusterInfos->push_back(
			    mu2e::IntensityInfoTimeCluster(*timeClusterInfo));
			if(_diagLevel > 2)
				std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber
				          << ":" << subrunNumber << ":" << eventNumber
	...
	if(_useTracker)
	{
		...
		if(!event.getByLabel(_trackerTag, trackerH) || !trackerH.product())
		{
			...
			_trackerInfos->push_back(
			    mu2e::
			        IntensityInfoTrackerHits());  // add an empty one to maintain the list alignment
		}
		else
		{
	...
	if(_useHeader)
	{
		...
		if(!event.getByLabel(_headerTag, headerH) || !headerH.product())
		{
			...
			_headers->push_back(
			    mu2e::EventHeader());  // add an empty one to maintain the list alignment
		}
		else
		{
	...
	{  // add empty information in failed events if writing into events
		if(_diagLevel > 2)
			std::cout << "LumiStreamFilter::" << __func__ << ": Event " << runNumber
			          << ":" << subrunNumber << ":" << eventNumber
		...
		if(_useTimeCluster)
		{
			auto tmp_info = std::unique_ptr<mu2e::IntensityInfosTimeCluster>(
			    new mu2e::IntensityInfosTimeCluster);
			event.put(std::move(tmp_info));
		}
		if(_useTracker)
		{
			auto tmp_info = std::unique_ptr<mu2e::IntensityInfosTrackerHits>(
			    new mu2e::IntensityInfosTrackerHits);
			event.put(std::move(tmp_info));
		}
		if(_useHeader)
